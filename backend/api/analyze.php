<?php
/**
 * POST /api/analyze.php   (يتطلب توكن: Authorization: Bearer <token>)
 * النقطة الأساسية: يستقبل صورة البشرة + إحداثيات GPS،
 * يجلب الطقس، يحلل بـ Gemini (رؤية + استشارة)، يحفظ الفحص، ويعيد النتيجة.
 *
...
// --- 4) تحليل الذكاء الاصطناعي (Gemini: رؤية + استشارة في طلب واحد) ---
...
$stmt = db()->prepare(
    'INSERT INTO scans
     (user_id, image_path, latitude, longitude, temperature, humidity, uv_index,
      weather_description, cv_detected_condition, cv_confidence_score, nlp_consultation_text)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
);
$stmt->execute([
